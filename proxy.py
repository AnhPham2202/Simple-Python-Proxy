...
def main():
    ...
    if len(sys.argv[1:]) != 5:
        ...
        print("# Open port 9000 ==> when traffics arrive port 9000 ==> forward it to [remotehost]:[remoteport]")
        sys.exit(0)
...
def proxy_handler(client_socket, remote_host, remote_port, receive_first):
    ...
    if receive_first:
        ...
        if len(remote_buffer):
            print("[<==] Sending %d bytes to localhost." % len(remote_buffer), client_socket.send(remote_buffer.encode()))
